...
if not image_files:
    ...
    for img_path in image_files:
        ...
        bw = morphology.remove_small_objects(bw, max_size=200)
        ...
        ax[1].imshow(tophat, cmap='gray'); ax[1].set_title('Tophat/Corrected'); ax[1].axis('off')
        ax[2].imshow(bw, cmap='gray'); ax[2].set_title('Binary (filled/cleaned)'); ax[2].axis('off')
        ax[3].imshow(clean_labels, cmap='nipy_spectral'); ax[3].set_title('Felzenszwalb + per-rock cleanup'); ax[3].axis('off')
        ax[4].imshow(km_seg, cmap='gray'); ax[4].set_title('LBP + k-means Clustering'); ax[4].axis('off')
        plt.tight_layout()
        plt.show()
